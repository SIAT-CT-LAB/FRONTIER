...
def map_coordinates(input, coordinates):
    ...
    co_floor = torch.floor(coordinates).long()  # 表示左下角的坐标
    co_ceil = torch.ceil(coordinates).long()  # 表示右上角的坐标
    d1 = (coordinates[1] - co_floor[1].float())
    d2 = (coordinates[0] - co_floor[0].float())
    co_floor = _coordinates_pad_wrap(h, w, co_floor)  # 防止新坐标越界（超过原图像范围）
...
def ct_parallel_project_2d(img, theta):
    ...
    x = x.cuda()
    y = y.cuda()
    # Rotation transform matrix: simulate parallel projection rays  旋转坐标系
    ...
    img_resampled = map_coordinates(img, sample_coords)  # [b, h, w, c]     # 得到的是以探测器板为x轴，x射线为y轴的图像

    # Compute integral projections along rays 对一条线求和
    proj = torch.mean(img_resampled, dim=0, keepdim=True)  # [b, 1, w, c]
...
def ct_parallel_project_2d_batch(img, thetas):
    ...
    for theta in thetas:
        ...
        projs.append(proj)
    projs = torch.cat(projs, dim=0)  # [b, num, w, c]
